...
} else if (!in_array($_POST['action'], array('edit', 'delete', 'comment', 'like', 'dislike', 'wonder', 'report', 'save', 'disable_comments', 'reaction', 'boost', 'pin', 'unpin'))) {
    $error_code = 6;
...
if (empty($error_code)) {
    ...
    } else if ($_POST['action'] == 'pin') {
        $post_id = Wo_Secure($_POST['post_id']);
        $post = Wo_PostData($post_id);
        if (Wo_IsPostOnwer($post_id, $wo['user']['user_id'])) {
            $result =  Wo_PinPost($post_id,'profile',$wo['user']['user_id']);
            if ($result == 'unpin') {
                $code =1;
                $action = 'post unpin';
            } else if ($result == 'pin') {
                $code = 1;
                $action = 'post pinded successfully';
            }
        } else {
            $error_code = 7;
            $error_message = 'You are not the post owner';
