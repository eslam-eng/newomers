...
if ($f == 'posts') {
    ...
    // Modified By A.Y
    if ($s == 'retweet-post' && isset($_GET['post_id']) && is_numeric($_GET['post_id']) && isset($_GET['user_id']) && is_numeric($_GET['user_id'])) {
        $data    = array(
            'status' => 304
        );
        $post_id = Wo_Secure($_GET['post_id']);
        $owner   = Wo_Secure($_GET['user_id']);
        if (AY_RetweetPostOn($post_id)) {
            $data['status'] = 200;
        }
        header("Content-type: application/json");
        echo json_encode($data);
